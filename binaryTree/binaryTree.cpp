...
template <class elemType>
void binaryTee<elemType>::preOrderTraversal(const treeNode<elemType> &root) {
...
template <class elemType>
vector<vector<int>> binaryTee<elemType>::levelOrder() {
    vector<vector<int>> v;
    levelOrderHelper(root, v, 0);
    return v;
}

template <class elemType>
void binaryTee<elemType>::levelOrderHelper(treeNode<elemType> *current, vector<vector<int>> &v, int i){
    if(!current){
        return;
    }else{
        cout<<"i: "<<i<<endl;
        v[i].push_back(current->info);
        i=i++;

        levelOrderHelper(current->llink, v, i);
        levelOrderHelper(current->rlink, v, i);
    }
}
